...
static void amiibolink_view_on_draw(mui_view_t *p_view, mui_canvas_t *p_canvas) {
    ...
    if (amd != NULL) {
        const char *name =(getLanguage() == LANGUAGE_ZH_TW || getLanguage() == LANGUAGE_ZH_HANS) && strlen(db_amiibo_get_name_cn(amd)) > 0 ? db_amiibo_get_name_cn(amd) : db_amiibo_get_name_en(amd);
