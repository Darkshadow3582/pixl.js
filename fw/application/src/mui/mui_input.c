...
static input_key_t mui_input_remap_key_for_display(input_key_t key) {
    const settings_data_t *p_settings = settings_get_data();
    if (!p_settings->display_flip) {
        return key;
    }
...
void mui_input_on_bsp_btn_event(uint8_t btn, bsp_btn_event_t evt) {
    input_key_t mapped_key = mui_input_remap_key_for_display((input_key_t)btn);
