...
void lcd_init(void) {
    ...
    lcd_write_command(0xA0); /*set segment remap - this board's OLED is mounted upside down*/
#else
...
#ifdef OLED_ROTATION_180
    lcd_write_command(0xC0); /*Com scan direction - this board's OLED is mounted upside down*/
#else
